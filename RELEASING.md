...
1. Prepare and validate a release-candidate branch such as
   `release/sophiagraph-<version>rc1`.
2. Push an RC tag such as `v<version>rc1`; RC tag pushes publish to TestPyPI only.
3. Install and smoke-test the RC artifact from TestPyPI.
4. Prepare and validate the final branch such as `release/sophiagraph-<version>`.
...
7. Push the final non-RC tag such as `v<version>`; final tag pushes publish to
   production PyPI.
8. Create the GitHub Release with the bare version title, for example `<version>`.
